...
class TestBuildAuthProvider:
    ...
    def test_build_auth_provider_with_direct_auth_config(self):
        ...
        with patch("itential_mcp.auth.JWTVerifier") as mock_jwt_verifier:
    ...
    @patch("itential_mcp.auth.JWTVerifier", side_effect=Exception("general error"))
...
class TestJWTProviderBuilder:
    ...
    @patch("itential_mcp.auth.JWTVerifier", side_effect=ValueError("invalid key"))
    ...
    @patch("itential_mcp.auth.JWTVerifier", side_effect=RuntimeError("runtime error"))
...
class TestOAuthProviderBuilder:
    ...
    @patch("itential_mcp.auth.OAuthProvider", side_effect=ValueError("invalid config"))
    ...
    @patch("itential_mcp.auth.OAuthProvider", side_effect=RuntimeError("runtime error"))
    ...
    def test_build_oauth_proxy_provider_minimal_config(self, mock_static_verifier, mock_oauth_proxy):
    ...
    def test_build_oauth_proxy_provider_missing_fields(self):
        ...
        assert "OAuth proxy authentication requires the following fields" in str(exc.value)
        for field in ["client_secret", "authorization_url", "token_url", "redirect_uri"]:
            assert field in str(exc.value)

    @patch("itential_mcp.auth.OAuthProxy")
    @patch("fastmcp.server.auth.StaticTokenVerifier")
    def test_build_oauth_proxy_provider_with_optional_fields(self, mock_static_verifier, mock_oauth_proxy):
        ...
        assert kwargs["upstream_revocation_endpoint"] == "https://auth.example.com/oauth/userinfo"
        assert kwargs["valid_scopes"] == ["openid", "email"]

    @patch("itential_mcp.auth.OAuthProxy")
    @patch("fastmcp.server.auth.StaticTokenVerifier", side_effect=ImportError)
    @patch("itential_mcp.auth.JWTVerifier")
    def test_build_oauth_proxy_provider_fallback_verifier(self, mock_jwt_verifier, mock_static_verifier, mock_oauth_proxy):
